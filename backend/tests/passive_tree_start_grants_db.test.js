...
test('start-node grants reach passive_nodes', { skip: !url ? 'no database URL' : false }, async (t) => {
  ...
  // SOMET-477: this file empties the six start nodes' grants and reseeds, and
  // passive_tree_seed_db.test.js runs a --force reseed in a parallel process
  // against the same database. Both rewrite the columns the other asserts on,
  // so all three passive-tree DB files hold this one key while they do it.
