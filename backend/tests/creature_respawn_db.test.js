...
//
// The world is removed even when an assertion throws -- see
// tests/helpers/fixtureWorld.js. That guarantee is the point: cleanup used to
// sit at the end of each `try`, so the first failing assertion leaked its
// world, and nine of them accumulated in the shared dev database from this
// file alone (SOMET-341).
const withWorld = (pool, fn) => withFixtureWorld(pool, fn, { prefix: 'respawn-test' });
...
test('a world with no allowed creature types enqueues nothing', { skip: !url }, async () => {
  const pool = new Pool({ connectionString: url });
  try {
    // allowed_creature_types left empty
    await withWorld(pool, async (worldId) => {
      const row = (await pool.query('SELECT * FROM worlds WHERE id = $1', [worldId])).rows[0];
