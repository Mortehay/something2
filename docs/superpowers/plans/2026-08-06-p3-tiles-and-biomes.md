...
test('exactly ten named biomes band impassable terrain', () => {
  ...
  assert.deepEqual(withBlocked, [
    ...
    'Mire', 'Pestilent Deep', 'Shattered Vault', "Titan's Grave", 'The Maw',
    'Umbral Warren',
  ].sort());
});
```
...
```js
  ...
  { name: "Titan's Grave", terrain_tiles: ['titan_floor', 'ruin_stone', 'rubble'], flora_types: ['Stone'], creature_types: [], palette: ['weathered granite', 'bone grey', 'dust gold'], art_style: 'colossal buried ruin, monumental scale', exclusions: 'no plants, no daylight, no water', color: '#7a7266' },
