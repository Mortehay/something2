...
- **Key-based access, two keys.** The Pi password is used once, by `pi-keygen`,
  to install the workstation key and scoped sudo. The Pi generates its own
  GitHub deploy key, whose private half never leaves the board. A read-only
  token remains as a fallback clone path.
...
Access involves two separate keypairs. Conflating them is the usual way this
goes wrong, so they are named apart everywhere.

| Key | Generated on | Public half installed in | Purpose |
|---|---|---|---|
| Workstation key | the workstation | the Pi's `authorized_keys` | password-free `make pi-*` |
| Deploy key | **the Pi** | GitHub repository deploy keys | the Pi clones and pulls |

The deploy key's private half is generated on the board and **never leaves it**.
It is read-only, scoped to one repository, revocable on its own and does not
expire. Nothing sensitive travels from `.env` to the Pi for git access.

Git authentication keeps **two paths: the deploy key is primary, the token is a
fallback.** `provision.sh` and `deploy.sh` try SSH first and fall back to HTTPS
with the token when SSH is unavailable. Both paths are exercised in testing —
a fallback that has never run is not a fallback.
...
ORANGEPI_PASSWORD     bootstrap ONLY; blanked after pi-keygen
...
GIT_SSH_URL           git@host:owner/repo.git -- primary clone path
GIT_REPOSITORY        https clone URL -- fallback path
GIT_USERNAME          account the token belongs to (fallback only)
GIT_TOKEN             Personal Access Token, read-only (fallback only)
GIT_BRANCH            default main
DEPLOY_HOOK_SECRET    HMAC shared secret for the CI-to-Pi deploy call
```

The Pi's deploy key is **not** an `.env` value. Its private half stays on the
board; its public half is written to
`compose/orangepi/secrets/github_deploy_key.pub` and printed. A public key is
not a secret, and burying it in a gitignored `.env` would hide it at exactly the
moment it needs to be copied into GitHub.
...
`ORANGEPI_PASSWORD` is deliberately short-lived. `pi-keygen` uses it once to
install the workstation key and grant `NOPASSWD` sudo scoped to `docker` and
...
the Pi and GitHub simultaneously.

1. Create the workstation key at `ORANGEPI_SSH_KEY` if absent; skip if present.
2. Install its public half into the Pi's `authorized_keys`. **The only step that
   uses `ORANGEPI_PASSWORD`.**
3. On the Pi, create the GitHub deploy key if absent; skip if present.
4. Fetch its public half back, write it to
   `compose/orangepi/secrets/github_deploy_key.pub`, and print it alongside the
   exact GitHub URL for adding a deploy key.
5. Run `ssh -T git@github.com` from the Pi and report whether the repository is
   reachable yet.

Step 5 is what turns "did I paste that correctly?" into an answered question,
rather than a clone that fails twenty minutes later inside provisioning.

**`provision.sh`** — idempotent bootstrap, safe to re-run:

1. Verify reachability and that `DATA_DIR` is not inside `APP_DIR`.
2. **Verify the repository is reachable.** If not, stop immediately and reprint
   the deploy key rather than failing partway through a clone.
...
6. Wipe and recreate `APP_DIR`; clone at `GIT_BRANCH` — `GIT_SSH_URL` via the
   deploy key by preference, falling back to `GIT_REPOSITORY` over HTTPS with
   the token. On the fallback path, rewrite the remote afterwards so the token
   is not persisted in git config.
...
| Target | Purpose |
|---|---|
| `pi-keygen` | Generate the workstation and deploy keys if absent; print the public deploy key to paste into GitHub |
...
- `make pi-keygen` produces both keys, prints the public deploy key, and
  reports whether the repository is reachable — and a second run regenerates
  nothing.
