...
**Host-deletion gap, called out explicitly (implementation detail, not a schema defect):** `ON DELETE SET NULL` fires when the *referenced* row — here, the stone's own `player_items` row — is deleted, not when the *host* `player_items` row (the weapon/armor being pointed at) is deleted. Dropping, selling, or otherwise deleting a host item while a stone is socketed into it needs an explicit step in that deletion path to null out the socketing stone's `socketed_into_id` first (ejecting it back to loose inventory), or the stone is left pointing at a nonexistent host. The implementation plan must include this as an explicit, tested step everywhere a host `player_items` row can be deleted (`dropItem`, `sellItem`, and any other deletion site — enumerate them at implementation time).
